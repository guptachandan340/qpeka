...
public class UserService {
	...
	public Response editBasicSocialProfileService(
		...
		try {
			...
			if (!sResponse.isEmpty() && sResponse != null) {
				return Response.status(200)
						.entity(new Gson().toJson(sResponse)).build();
			} else {
				return Response.status(200).entity(new Gson().toJson(""))
						.build();
			}
		} catch (UserException e) {
			// TODO Auto-generated catch block
	...
	public Response createuserInvites(
		...
		long userid = Long.parseLong(inviteParams.get("userid").iterator()
				.next());
		return Response
				.status(200)
				.entity(new Gson().toJson(UserInvitesManager.getInstance()
						.createUserInvites(userid, inviteParams))).build();
	}
	...
	public Response userInviteSent(MultivaluedMap<String, String> hashvalues)
		...
		return Response
				.status(200)
				.entity(new Gson().toJson(UserInvitesManager.getInstance()
						.setInviteSent(hashvalues.get("hashvalues")))).build();
	}
