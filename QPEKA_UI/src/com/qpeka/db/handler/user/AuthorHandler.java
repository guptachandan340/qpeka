...
public class AuthorHandler {
	...
	public String addAuthor(Author author, boolean useId) {
		BasicDBObject dObj = (BasicDBObject) author.toDBObject(useId ? true
				: false);
		WriteResult result = authors.insert(dObj, WriteConcern.SAFE);
		ObjectId id = dObj.getObjectId("_id");
		return id.toString();
	...
	public void updateAuthor(Author author) {
		...
		authors.update(q, new BasicDBObject("$set", author.toDBObject(true)),
	...
	public List<Author> getAuthorsByLikelyName(String query) {
		...
		list.add(new BasicDBObject(Author.NAME + "." + Name.FIRSTNAME, "/"
				+ query + "/"));
		list.add(new BasicDBObject(Author.NAME + "." + Name.MIDDLENAME, "/"
				+ query + "/"));
		list.add(new BasicDBObject(Author.NAME + "." + Name.LASTNAME, "/"
				+ query + "/"));
	...
	public static void main(String[] args) {
		 ...
		 * Author a = null;
		 * 
		 * for(int i = 0 ;i < 50 ; i ++) { a= new Author(); a.setName(new
		 * Name("manoj"+ i, "ramesh" +i, "thakur" + i)); a.setDob(new Date());
		 * a.setGender(GENDER.MALE); a.setInfoLink("http://google.com"+i);
		 * a.setImageFile("/tmp/img"+i+".jpg"); JSONArray j = new JSONArray();
		 * j.put(CATEGORY.FICTION); j.put(CATEGORY.SUSPENSE); a.setGenre(j);
		 * a.setShortBio("BIO "+i);
		 * 
		...
		for (int i = 0; i < 10; i++) {
			PublisherHandler.getInstance().addPublisher(
					new Publisher("TMH " + i, ""));
		}
